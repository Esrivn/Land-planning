...
const Widget = (props: AllWidgetProps<IMConfig>): React.ReactElement => {
  ...
  const getLanText = () => {
    let url = window.location.href;
      ...
      const lan = url.substr(idx, url.length);
      let title = config.titleEN;
      if (lan === "locale=en") {
        title = config.titleEN
      } 

      if (lan === "locale=vi") {
        title = config.titleVI
      } 
      return title;
  }
  ...
  let textHTML = new DOMParser().parseFromString(text, "text/xml");
  if (textHTML && textHTML.children[0] && textHTML.children[0].children[0]) {
    text = textHTML;
    text.children[0].children[0].innerHTML = getLanText();
    text = text.documentElement.outerHTML;
  }
  
  // const text = "<p><span style='color: var(--dark);'>Tiêu đề</span></p>"
